...
namespace cherrydev
{
    public class NodeEditor : EditorWindow
    {
        ...
        private GUIStyle setupPanelStyle;
        ...
        private const float nodeHeight = 350f;
        ...
        private void OnEnable()
        {
            ...
            setupPanelStyle = new GUIStyle();
            setupPanelStyle.normal.background = EditorGUIUtility.Load(StringConstants.Node) as Texture2D;
            setupPanelStyle.padding = new RectOffset(nodePadding, nodePadding, nodePadding, nodePadding);
            setupPanelStyle.border = new RectOffset(nodeBorder, nodeBorder, nodeBorder, nodeBorder);
        ...
        private void DrawGridBackground(float gridSize, float gridOpacity, Color color)
        {
            ...
            DrawSetupPanel(gridSize, gridOpacity);
        }
        private void DrawSetupPanel(float gridSize, float gridOpacity)
        {
            float panelWidth = 220f;
            float panelHeight = 180f;

            Rect panelRect = new Rect(position.width - panelWidth - 10f, 10f, panelWidth, panelHeight);
            GUI.backgroundColor = new Color(0.4f, 0.4f, 0.4f, 1f);
            GUIStyle panelStyle = new GUIStyle("box")
            {
                normal = { background = MakeTexture(1, 1, new Color(0.4f, 0.4f, 0.4f)) },
            ...
            GUILayout.Label("Setup", headerStyle);
            GUILayout.Space(10f);

            EditorGUILayout.BeginHorizontal();
            currentNodeGraph.blankPlayerIcon = EditorGUILayout.ObjectField("Blank Player Icon", currentNodeGraph.blankPlayerIcon, typeof(RenderTexture), false) as RenderTexture;
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            currentNodeGraph.blankNpcIcon = EditorGUILayout.ObjectField("Blank Npc Icon", currentNodeGraph.blankNpcIcon, typeof(RenderTexture), false) as RenderTexture;
            EditorGUILayout.EndHorizontal();
            ...
            GUIStyle buttonStyle = new GUIStyle(GUI.skin.button)
            {
                fontSize = 12,
                alignment = TextAnchor.MiddleCenter,
                normal = { background = MakeTexture(1, 1, new Color(0.1f, 0.1f, 0.1f)) }, // Dark background for the button
                hover = { background = MakeTexture(1, 1, new Color(1f, 1f, 0f)) }, // Yellow background on hover
            };
        ...
        private void SelectNodesBySelectionRect(Vector2 mousePosition)
        {
            ...
            EditorGUI.DrawRect(selectionRect, new Color(0.6f, 0.5f, 0f, 0.5f));
