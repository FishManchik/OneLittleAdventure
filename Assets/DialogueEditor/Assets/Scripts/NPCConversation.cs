...
namespace DialogueEditor
{
    ...
    public class NPCConversation : MonoBehaviour
    {
        ...
        [SerializeField] public PlayableDirector DefaultDirector;
        ...
        private Conversation ConstructConversationObject(EditableConversation ec)
        {
            ...
            Dictionary<int, CutsceneNode> cutscenesByID = new Dictionary<int, CutsceneNode>();
            ...
            for (int i = 0; i < ec.Cutscenes.Count; i++)
            {
                CutsceneNode node = CreateCutsceneNode(ec.Cutscenes[i]);
                cutscenesByID.Add(ec.Cutscenes[i].ID, node);
            }

            // Now that we have every node in the dictionary, reconstruct the tree 
            // And also look for the root
            ReconstructTree(ec, conversation, speechByID, optionsByID, cutscenesByID);
        ...
        private SpeechNode CreateSpeechNode(EditableSpeechNode editableNode)
        {
            ...
            speech.CharacterIcon = editableNode.CharacterIcon;
            speech.Audio = editableNode.Audio;
            speech.Cutscene = editableNode.Cutscene;
            speech.Volume = editableNode.Volume;
            speech.IsPlayerDialogue = editableNode.IsPlayerDialogue;
        ...
        private CutsceneNode CreateCutsceneNode(EditableCutsceneNode editableNode)
        {
            CutsceneNode speech = new CutsceneNode();
            speech.Cutscene = editableNode.Cutscene;
        ...
        private OptionNode CreateOptionNode(EditableOptionNode editableNode)
        {
            ...
            option.Cutscene = editableNode.Cutscene;
        ...
        private void ReconstructTree(EditableConversation ec, Conversation conversation, Dictionary<int, SpeechNode> dialogues, Dictionary<int, OptionNode> options, Dictionary<int, CutsceneNode> cutscenes)
        {
            // Speech nodes
